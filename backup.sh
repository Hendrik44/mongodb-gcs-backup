...
EOF
  ...
  gsutil cp $BACKUP_DIR/$archive_name gs://$GCS_BUCKET/$archive_name
  echo "verify that transfered file exist in bucket because google is saying ok but sometimes there is nothing..."
  gsutil stat gs://$GCS_BUCKET/$archive_name
}
